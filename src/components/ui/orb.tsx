...
export const Component: FC<ComponentProps> = ({
  ...
  useEffect(() => {
    ...
    try {
        ...
        rendererInstance.gl.clearColor(0, 0, 0, 0);
        
        if (container.firstChild) {
            container.removeChild(container.firstChild);
        }
        container.appendChild(rendererInstance.gl.canvas);

        const geometry = new Triangle(rendererInstance.gl);
        const program = new Program(rendererInstance.gl, {
        vertex: vert,
        fragment: frag,
        uniforms: {
            iTime: { value: 0 },
            iResolution: {
            value: new Vec3(
                rendererInstance.gl.canvas.width,
                rendererInstance.gl.canvas.height,
                rendererInstance.gl.canvas.width / rendererInstance.gl.canvas.height
            ),
            },
            hue: { value: hue },
            hover: { value: 0 },
        ...
        const mesh = new Mesh(rendererInstance.gl, { geometry, program });
        ...
        const resize = () => {
            ...
            rendererInstance.gl.canvas.style.width = width + "px";
            rendererInstance.gl.canvas.style.height = height + "px";
            
            program.uniforms.iResolution.value.set(
                rendererInstance.gl.canvas.width, // Use drawing buffer width/height
                rendererInstance.gl.canvas.height,
                rendererInstance.gl.canvas.width / rendererInstance.gl.canvas.height
            );
        };
        ...
        let lastTime = 0;
        let currentRot = 0;
        const rotationSpeed = 0.05; // Slower rotation to match external nodes
        ...
        return () => {
          ...
          if (container) {
            ...
            if (rendererInstance && rendererInstance.gl.canvas && rendererInstance.gl.canvas.parentNode === container) {
              container.removeChild(rendererInstance.gl.canvas);
            }
          }
          if (rendererInstance) {
